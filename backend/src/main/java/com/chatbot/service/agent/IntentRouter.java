...
public class IntentRouter {
    ...
    private static final String INTENT_SYSTEM_PROMPT = """
            ...
            可识别的意图类型：
            - POST_QUERY: 用户想查询某个**具体用户**的帖子/文章（必须能提取到明确的 username 参数）。如果用户没有指定具体用户名，不要归类为 POST_QUERY。
            - DATA_DELETION: 用户想删除自己的数据（需要提取 username 参数）
            - KB_QUESTION: 用户在问产品、服务、政策、功能、操作方法等方面的问题。包括但不限于：退款、退货、密码重置、账号安全、两步验证、支付方式、配送、会员等一切业务相关问题。只要用户在问一个业务/产品/服务相关的问题，就应该归类为 KB_QUESTION。
            ...
            **重要**：当不确定时，优先归类为 KB_QUESTION 而非 GENERAL_CHAT。只有纯闲聊才归为 GENERAL_CHAT。
            ...
            用户说"有谁的帖子" → {"intent":"KB_QUESTION","confidence":0.7,"risk":"low","extracted_params":{}}
            用户说"帮我查一下帖子" → {"intent":"POST_QUERY","confidence":0.7,"risk":"low","extracted_params":{}}
